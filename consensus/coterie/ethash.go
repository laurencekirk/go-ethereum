...
var (
	...
	// sharedCoterie is a full instance that can be shared between multiple users.
	sharedCoterie = New("", 3, 0, "", 1, 0)
...
// Coterie is a consensus engine based on proot-of-work implementing the ethash
// algorithm.
type Coterie struct {
	...
	shared    *Coterie       // Shared PoW verifier to avoid cache regeneration
...
// consensus rules.
func NewFaker() *Coterie {
	return &Coterie{fakeMode: true}
}

// NewFakeFailer creates a ethash consensus engine with a fake PoW scheme that
// accepts all blocks as valid apart from the single one specified, though they
// still have to conform to the Ethereum consensus rules.
func NewFakeFailer(fail uint64) *Coterie {
	return &Coterie{fakeMode: true, fakeFail: fail}
}

// NewFakeDelayer creates a ethash consensus engine with a fake PoW scheme that
// accepts all blocks as valid, but delays verifications by some time, though
// they still have to conform to the Ethereum consensus rules.
func NewFakeDelayer(delay time.Duration) *Coterie {
	return &Coterie{fakeMode: true, fakeDelay: delay}
}

// NewFullFaker creates an ethash consensus engine with a full fake scheme that
// accepts all blocks as valid, without checking any consensus rules whatsoever.
func NewFullFaker() *Coterie {
	return &Coterie{fakeMode: true, fakeFull: true}
}

// NewShared creates a full sized ethash PoW shared between all requesters running
// in the same process.
/*func NewShared() *Coterie {
	return &Coterie{shared: sharedCoterie}
}*/

// cache tries to retrieve a verification cache for the specified block number
// by first checking against a list of in-memory caches, then against caches
// stored on disk, and finally generating one if none can be found.
/*func (ethash *Coterie) cache(block uint64) []uint32 {
...
// stored on disk, and finally generating one if none can be found.
/*
func (ethash *Coterie) dataset(block uint64) []uint32 {
	epoch := block / epochLength
...
/*
func (ethash *Coterie) Threads() int {
	ethash.lock.Lock()
...
// work being done.
/*
func (ethash *Coterie) SetThreads(threads int) {
	ethash.lock.Lock()
	defer ethash.lock.Unlock()
...
/*
func (ethash *Coterie) Hashrate() float64 {
...
// that is empty.
/*func (ethash *Coterie) APIs(chain consensus.ChainReader) []rpc.API {
	return nil
}*/
